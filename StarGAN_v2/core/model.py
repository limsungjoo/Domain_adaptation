...
class UNet(nn.Module):
    ...
    def encode(self, x):
        ...
        x5 = self.down4(x4)
        return x5
    def decode(self,x,x1,x2,x3,x4,x5):
        x = self.up1(x5, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
...
class UnetGenerator(nn.Module):
	def __init__(self, in_channels, n_classes, base_n_filter = 2):
		...
		# self.coordconv1 = CoordConv2d(2, 2, kernel_size=1,stride=1, padding=0, dilation=1, groups=1, bias=True, with_r=False, use_cuda=True)
...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512, w_hpf=1):
        ...
        for _ in range(repeat_num):
            ...
            self.encode.append(
                # UNet.encode(dim_in,dim_out))
                UnetGenerator(dim_in, dim_out,2))
